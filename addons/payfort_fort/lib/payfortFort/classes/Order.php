...
class Payfort_Fort_Order
{
    ...
    {
        return isset($this->order['payment_method']['params']['integration_type']) ? $this->order['payment_method']['params']['integration_type'] : PAYFORT_FORT_INTEGRATION_TYPE_REDIRECTION;
    }
    ...
    public function declineOrder($reason)
    {
        ...
        if(empty($orderPlacement) || $orderPlacement == 'all'){
            ...
            $payment_id = db_get_field("SELECT payment_id FROM ?:orders WHERE order_id = ?i", $this->getOrderId());

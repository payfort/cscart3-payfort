...
//    }
    ...
    $integration_type = isset($processor_data['params']['integration_type']) ? $processor_data['params']['integration_type'] : '';
    if($payment_method == 'payfort_fort_cc' && $integration_type == 'merchantPage') {
        ...
        fn_payfort_fort_submit_form($gatewayUrl, $post_data);
    }
}
...
function fn_payfort_fort_convert_fort_amount($amount, $currency_code)
{
    ...
    //$total = fn_format_price($amount, CART_SECONDARY_CURRENCY);
    $total = fn_format_rate_value($amount, 'F', $currency_data['decimals'], '.', '', $currency_data['coefficient']);
    //$total = fn_format_price_by_currency($amount, CART_SECONDARY_CURRENCY);
